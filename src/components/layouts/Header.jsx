...
import {
  ...
  RiInformationLine, RiLoader4Line, RiFlag2Line, 
  RiTaskLine, RiAlertLine, RiChat1Line
} from 'react-icons/ri';
  ...
  const SearchResultItem = ({ item }) => {
    ...
    return (
      <Link 
        ...
        onClick={() => { setShowDropdown(false); setKeyword(''); setMobileMenuOpen(false); }}
      >
            ...
            <div className="hidden md:block relative" ref={searchRef}>
              ...
              {showDropdown && keyword.length >= 2 && (
                <div className="absolute top-full mt-2 left-0 w-64 lg:w-80 bg-[#1a1a2e] border border-white/10 rounded-xl shadow-2xl p-2 z-10 animate-fade-in-up">
                  {isSearching ? (
                    <div className="p-3 text-center text-gray-500 text-xs">Đang tìm kiếm...</div>
                  ) : searchResults.length > 0 ? (
                    <div className="flex flex-col">
                      {searchResults.map(item => (<SearchResultItem key={item._id} item={item} />))}
                      <div className="border-t border-white/5 mt-2 pt-2">
                        <button onClick={handleSearchSubmit} className="w-full text-center text-xs text-primary font-bold py-2 hover:underline">Xem tất cả kết quả cho "{keyword}"</button>
                      </div>
                    </div>
                  ) : (
                    <div className="p-3 text-center text-gray-500 text-xs">Không tìm thấy truyện nào.</div>
                  )}
                </div>
              )}
            </div>
                      ...
                      {notifications.length > 0 ? notifications.map(notif => {
                        ...
                        return (
                          <Link 
                            ...
                            className={`flex gap-3 p-3 border-b border-white/5 transition-colors border-l-2 ${notif.is_read ? 'opacity-60 hover:bg-white/5 border-transparent' : `bg-white/5 ${style.bgHover} ${style.borderClass}`}`}
                          >
                  ...
                  <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 bg-[#252538] hover:bg-white/10 border border-white/5 rounded-full py-1 pr-3 pl-1 transition-all">
                    <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-700 border border-white/10">
                      {user.avatar ? 
                        ...
                        <div className="w-full h-full flex items-center justify-center font-bold text-white bg-gradient-to-tr from-primary to-purple-600">{user.full_name?.charAt(0).toUpperCase()}</div>
                      }
                    </div>
          ...
          <div className="p-4 flex flex-col gap-3">
            {user && (
              <div className="flex items-center gap-3 bg-[#252538] p-3 rounded-xl border border-white/5 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1"><LevelBadge exp={user.exp} rankStyle={user.rank_style} role={user.role} /></div>
                </div>
              </div>
            )}
            ...
            <div className="relative" ref={searchRef}>
              <div className="flex items-center bg-[#252538] rounded-xl px-4 py-3 mb-2 border border-white/5 focus-within:border-primary/50 transition-colors relative">
                ...
                <input 
                  ...
                  className="bg-transparent border-none focus:outline-none text-sm text-white px-3 w-full" 
                  value={keyword} 
                  onChange={(e) => setKeyword(e.target.value)} 
                  onKeyDown={handleKeyDown} 
                  onFocus={() => keyword.length >= 2 && setShowDropdown(true)} 
            ...
            {user ? (
              <>
                ...
                {user.role === 'admin' && (<Link to="/admin" className="text-red-500 font-bold p-3 rounded-xl hover:bg-red-500/10 transition-colors flex items-center gap-2" onClick={() => setMobileMenuOpen(false)}><RiAdminLine /> Quản Trị Viên</Link>)}
