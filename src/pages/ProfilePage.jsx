...
const TaskItem = ({ task, onClaim }) => {
    ...
    return (
        <div className="bg-[#1f1f3a] p-4 rounded-xl border border-white/5 flex items-center justify-between group hover:border-white/10 transition-colors">
...
const ProfilePage = () => {
    ...
    // States
    const [activeTab, setActiveTab] = useState('info'); // info | tasks | password
                                    ...
                                    {user.role === 'user' ? (
                                        <>
                                            <div className="w-full h-3 bg-[#1f1f3a] rounded-full overflow-hidden border border-white/5">
                                                <div className="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-1000 ease-out" style={{ width: `${progressPercent}%` }}></div>
                                            </div>
                            ...
                            {activeTab === 'tasks' && user.role === 'user' && (
                                <div className="space-y-10 animate-fade-in-up">
                                        ...
                                        {taskLoading && tasks.length === 0 ? <div className="text-center"><RiLoader4Line className="animate-spin text-2xl mx-auto"/></div> : (
                                            <div className="grid gap-3">
                                                {tasks.filter(t => t.type === 'daily').length > 0 
                                                    ? tasks.filter(t => t.type === 'daily').map(t => <TaskItem key={t.id} task={t} onClaim={handleClaim} />) 
                                                    : <p className="text-gray-500 text-xs italic">Không có nhiệm vụ ngày.</p>}
                                            </div>
                                        )}
                                    </div>
                                    {/* Weekly */}
                                    <div>
                                        ...
                                        <div className="grid gap-3">
                                            {tasks.filter(t => t.type === 'weekly').length > 0 
                                                ? tasks.filter(t => t.type === 'weekly').map(t => <TaskItem key={t.id} task={t} onClaim={handleClaim} />) 
                                                : <p className="text-gray-500 text-xs italic">Không có nhiệm vụ tuần.</p>}
                                        </div>
                                    </div>
                                    {/* Achievement */}
                                    <div>
                                        ...
                                        <div className="grid gap-3">
                                            {tasks.filter(t => t.type === 'achievement').length > 0 
                                                ? tasks.filter(t => t.type === 'achievement').map(t => <TaskItem key={t.id} task={t} onClaim={handleClaim} />) 
                                                : <p className="text-gray-500 text-xs italic">Chưa mở khóa thành tựu.</p>}
                                        </div>
                                    </div>
                                </div>
                            )}
