...
import {
  ...
  RiBookmarkFill, RiPlayCircleLine, RiFlag2Line, RiCloseLine, RiCheckLine
} from 'react-icons/ri';
...
const ReportModal = ({ isOpen, onClose, onSubmit, reason, setReason, isSent }) => {
        ...
        {!isSent ? (
          <>
            ...
            <div className="space-y-2 mb-6">
              {['Thông tin sai lệch', 'Ảnh bìa lỗi', 'Chapter lỗi', 'Trùng lặp', 'Khác'].map((r) => (
            ...
            <button 
              onClick={onSubmit} 
              disabled={!reason} 
              className={`w-full py-3 rounded-lg font-bold text-sm transition-all ${reason ? 'bg-primary text-white shadow-lg hover:bg-blue-600' : 'bg-gray-700 text-gray-500 cursor-not-allowed'}`}
            >
              Gửi Báo Cáo
            </button>
          </>
...
// Main page component: ComicDetailPage
  ...
  useEffect(() => {
    const fetchComicDetail = async () => {
      try {
        ...
        // Parallel Fetch: Rating & Comic Info
        const [ratingRes, comicRes] = await Promise.all([
          axios.get(`${BACKEND_URL}/api/rating/comic/${slug}?userId=${userId}`),
        ...
        // Fetch User Specific Data (Follow/History)
        if (user) {
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      ...
      await axios.post(`${BACKEND_URL}/api/reports`, 
        { comic_slug: slug, chapter_name: 'Trang Chi Tiết', reason: reportReason }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setReportSent(true);
      setTimeout(() => {
        ...
        <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-end">
          ...
          <div className="flex-1 text-center md:text-left flex flex-col gap-3 w-full">
            ...
            <p className="text-sm text-gray-400 line-clamp-2">
              ...
              {comic.origin_name && comic.origin_name.length > 0 ? comic.origin_name.join(' | ') : comic.name}
            </p>
            ...
            <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 md:gap-8 text-sm mt-2 bg-[#1a1a2e]/80 backdrop-blur-sm p-3 rounded-lg border border-white/5 w-fit mx-auto md:mx-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-300 text-xs md:text-sm">{comic.updatedAt ? new Date(comic.updatedAt).toLocaleDateString('vi-VN') : 'N/A'}</span>
              </div>
            </div>
